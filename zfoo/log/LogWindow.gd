## Overlay window that tails `{user_data}/logs/godot.log`.
## Dismiss with Esc, the close button, or a click outside (window loses focus).
...
static func show_log(line_count: int, width_percent: int, height_percent: int) -> void:
	...
	window.text_edit.text = LoggerHelper.tail_log(maxi(line_count, 1))
	window.text_edit.scroll_vertical = window.text_edit.get_line_count()
