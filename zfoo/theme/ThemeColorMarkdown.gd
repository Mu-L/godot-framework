...
## paints (code blocks, inline code chips, tables, quotes, links) follows the accent the user
## picked and the dark/light theme, instead of carrying its own hex constants.
## Usage: `ThemeColorMarkdown.code_block_bg` / `table_grid_color` / … ; `MarkdownUtils`
## turns each one into a BBCode argument at conversion time.
