...
description: >-
  ...
  user wants video-to-60fps, 鎻掑抚, frame interpolation, RIFE, 24 to 60, 30 to 60,
...
| Already ~60fps (59.5鈥?0.5, includes 59.94) | **Skip** 鈥?no output file |
...
2. **Already ~60?** 鈫?skip (do nothing).
3. **Above 60?** 鈫?FFmpeg drop to 60fps 鈫?`video-to-60fps/`.
4. **Below 60?** 鈫?Video2X RIFE (`-m` integer multiplier) at source resolution 鈫?FFmpeg to exact 60fps MP4 鈫?`video-to-60fps/`.
...
```
assets/video/clip.mp4          (1280脳720 24fps)
  鈫?assets/video/video-to-60fps/clip.mp4  (1280脳720 60fps)
```

Already-60fps source 鈫?no output file; log `[skip]`.
...
| Multiplier | Auto | Smallest integer `-m` so `src_fps 脳 m` is at least ~60 |
| UHD mode | Auto | `--uhd` when width 鈮?1920 |
| Already ~60fps | Skipped | No file written |

## Common Flags

`--video` 路 `-o` / `--output` 路 `--rife-model` 路 `--uhd` 路 `--gpu`
...
5. Video2X `-m` is an integer. 30fps 鈫?脳2 = 60. 24fps 鈫?脳3 = 72, then FFmpeg snaps to 60.
