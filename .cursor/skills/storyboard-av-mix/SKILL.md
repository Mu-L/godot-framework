...
description: >-
  ...
  (matched by shot id 01, 02, ‚Ä?. Use when the user wants storyboard A/V mix,
  ÂàÜÈïúÈü≥ËßÜÈ¢ëÂêàÊà? ÈÖçÈü≥ÂêàÊàê, video retime to audio, Video-Chinese, Video-English,
...
2. **Video serves audio.** Only change video duration (FFmpeg `setpts`) so it equals that shot‚Äôs VO length, then mux.
3. Drop all audio from the source video; replace with the VO track. Prefer `-c:a copy`; if the container cannot hold the VO codec (e.g. WAV‚ÜíMP4), encode AAC **without** changing length.
...
5. Match shots by **filename stem** (`01.mp4` ‚Ü?`01.wav`).
6. Batch only via `.ai/storyboard-av-mix/mix.py` ‚Ä?do not hand-write FFmpeg mux/retime commands.
...
2. Applies `setpts=PTS*(vo_dur/video_dur)` ‚Ä?stretch or compress **video only** (plus short freeze-tail so video ‚â?VO)
...
Typical `<root>` is a [storyboard-tts](../storyboard-tts/SKILL.md) audio dir that also has a sibling or nested `Video/` of cut clips ‚Ä?confirm the folder that contains `Video/`, `Chinese/`, and `English/`.
...
Existing outputs are overwritten. Missing VO for a language ‚Ü?skip that job with a warning. Missing video ‚Ü?skip shot.

## Agent Notes

1. Audio first: if durations disagree, change **video**, never VO.
2. Prefer one `mix.py` run for the whole board.
3. Re-encode must preserve source quality tags (Main10 / HDR / bitrate) ‚Ä?never force 8-bit H.264.
...
6. Missing Python/FFmpeg ‚Ü?populate `.dependency/` per skill-dependency-manager, retry same command.
...
- [storyboard-tts](../storyboard-tts/SKILL.md) ‚Ä?bilingual VO under `Chinese/` / `English/`
- [storyboard](../storyboard/SKILL.md) ‚Ä?source markdown
