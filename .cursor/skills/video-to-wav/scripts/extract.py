#!/usr/bin/env python3
"""Batch extract audio from video files to PCM WAV using FFmpeg."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch extract audio from video files to PCM WAV."
    )
...
def describe_file_plan(
    ...
    return f"track {track}, {rate}, {depth}, {ch}"
...
def main() -> int:
    ...
    for file_path in files:
        ...
        probe = probe_audio(ffprobe, file_path, args.track)
        if not probe:
            print(f"[fail] {rel}")
            print(
                f"No audio track at index {args.track} in: {file_path}",
                file=sys.stderr,
            )
            fail += 1
            continue
        ...
        stream_copy = can_stream_copy(
            probe, forced_rate, forced_depth, forced_channels
        )
