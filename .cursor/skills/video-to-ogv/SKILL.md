...
Lossy sources always go through a lossless intermediate first, then OGV 鈥?to minimize generation loss.
...
assets/video/intro.mp4
  鈫?assets/video/video-to-ogv/lossless/intro.mkv   (FFV1+FLAC intermediate)
  鈫?assets/video/video-to-ogv/intro.ogv            (final Godot asset)
```
...
|---------|---------|-------|
| Pipeline | Lossless 鈫?OGV | Skipped when source is already lossless |
...
Lossless intermediates are **much larger** than source MP4s (720p 鈮?50鈥?00 MB per 10 s clip). They are kept in `video-to-ogv/lossless/` for re-encoding. Delete manually when done, or pass **`--clean-lossless`** to remove after each successful OGV export.

## Common Flags

`--video` 路 `-o` / `--output` 路 `--lossless-dir` 路 `--clean-lossless` 路 `--standardize`
...
2. **Always run with no extra quality flags** 鈥?the script only supports the lossless MKV 鈫?OGV pipeline.
3. **Do not downscale or change frame rate** unless the user explicitly asks.
4. **Do not resample audio or change channel layout** unless the user explicitly asks 鈥?omit `--standardize`.
...
8. Need **48 kHz audio** 鈫?pass `--standardize`.
