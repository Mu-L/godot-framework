...
func StringBuilder_truncate_by_part_test() -> void:
	var builder := StringBuilder.new()
	builder.append_line("aaa").append_line("bbb").append_line("ccc")
	assert(builder.truncate_by_part(8))
	assert(builder.size() == 2)
	...
	builder.append_line("aaa").append_line("bbb").append_line("ccc")
	assert(builder.truncate_by_part(100) == false)
	assert(builder.size() == 3)
	assert(builder.build_string() == "aaa\nbbb\nccc\n")

	builder = StringBuilder.new(["0123456789"])
	assert(builder.truncate_by_part(5))
	assert(builder.size() == 1)
	assert(builder.build_string() == "01...")

	builder = StringBuilder.new()
	builder.append_line("keep-me").append_line("drop-me")
	assert(builder.truncate_by_part(0))
	assert(builder.is_empty())

	builder = StringBuilder.new()
	assert(builder.truncate_by_part(999) == false)
	assert(builder.is_empty())

	builder = StringBuilder.new()
	builder.append_line("12").append_line("34")
	assert(builder.truncate_by_part(6) == false)
	assert(builder.size() == 2)
	...
	assert(builder.truncate_by_part(35))
	assert(builder.size() == 3)
	assert(
	...
	builder.append_line("grep: zfoo/common/StringBuilder.gd:88:func truncate_by_part(max_length: int)")
	builder.append_line("... (truncated at 500 entries)")
	assert(builder.truncate_by_part(100))
	assert(builder.size() == 2)
	...
	assert(builder.truncate_by_part(80))
	...
	assert(builder.truncate_by_part(24))
	assert(builder.size() == 2)
	...
	assert(builder.truncate_by_part(20))
	assert(builder.size() == 1)
