...
func Alert_create_card_test() -> void:
	...
	assert(card.label.get_theme_font_size("font_size") == TextSize.body_large_size)
	assert(card.label.get_theme_font("font") is FontVariation)
	assert(card.label.get_theme_color("font_color") == ThemeColor.title_color)
...
func Alert_single_line_test() -> void:
	...
	var text_height: float = style.get_margin(SIDE_TOP) + style.get_margin(SIDE_BOTTOM) + Alert.make_font().get_height(TextSize.body_large_size)
