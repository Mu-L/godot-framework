...
func accept_segments(segments: Array[String]) -> void:
	...
	for segment in segments:
		var display := CharStreamUtils.truncate_at_punctuation(segment.strip_edges(), CharStreamUtils.MAX_PHRASE_LEN)
		if display.length() < CharStreamUtils.MIN_PHRASE_LEN:
			continue
		if char_queue.size() >= CHAR_QUEUE_CAP:
			char_queue.pop_front()
		char_queue.append(display)
		if keywords.size() < keyword_cap and not seen.has(display):
			seen[display] = true
...
func spawn_segment(segment: String) -> void:
	...
	particle.reset(
		...
		display_color,
		speed,
		rng.randi_range(0, maxi(neuron_net.get_positions().size() - 1, 0))
	)
...
func update_particles(delta: float) -> void:
	...
	while i < active_particles.size():
		if active_particles[i].update(delta):
			i += 1
			continue
		free_labels.append(active_particles[i].label)
...
func spawn_keywords(words: Array[String]) -> void:
	...
	for word in words:
		...
		label.position = Vector3(cos(angle) * radius, rng.randf_range(-0.45, 0.55), sin(angle) * radius * 0.55)
...
func update_keywords(delta: float) -> void:
	for label in keyword_labels:
		...
		var alpha: float = sin(clampf(age / life, 0.0, 1.0) * PI) * 0.75
