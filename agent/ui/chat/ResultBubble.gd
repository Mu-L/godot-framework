...
static func append(
	...
	title_label.text = I18n.t("agent.chat.title.result") if entry.title == ChatEntry.TITLE_RESULT else entry.title
...
static func style_view_button(button: Button) -> void:
	button.tooltip_text = I18n.t("agent.chat.view_result")
